...
// 测活页「配置哪些站点使用此测活词」弹窗（Phase G）：
// 默认平铺 / 分组两视图；勾选态本地暂存（切视图不丢），保存时统一落 sites[].probeText。
...
// 本次会话的勾选态（siteName → checked）。打开时按 s.probeText===probeText 初始化。
...
watch(
  ...
  ([open, text]) => {
    ...
    sitesState.list.forEach((s) => { init[s.name] = s.probeText === text; });
    checks.value = init;
  },
  { immediate: true },
);
...
function onSave() {
  if (props.probeText == null) { emit('close'); return; }
  const checkedNames = new Set(
    Object.entries(checks.value).filter(([, v]) => v).map(([k]) => k),
  );
  const cnt = assignSitesToProbe(props.probeText, checkedNames);
