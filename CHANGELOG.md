...
- **填了 Access Token 保存站点报 HTTP 500（重要）**：根因是文档与代码的格式要求不一致——`ENCRYPTION_KEY` 必须是 **base64 编码的 32 字节**，但文档（README 三语、`.env.example`）此前让用户用 `openssl rand -hex 32` 生成，产出的是 64 字符十六进制串，解码得 48 字节。十六进制字符恰好全落在 base64 字符集内，所以解码不报错、只是静默解出错误长度，坏密钥能一路混过部署与登录，直到用户第一次保存带 Access Token 的站点才炸成没有响应体的裸 500（不填 token 则不触发加密，一切正常，因此极难自查）。`SESSION_SECRET` 用十六进制串是无害的（HMAC 接受任意长度密钥），这也是"能正常登录却存不了 token"的原因。三处一并修：
  ...
  > **已按旧文档部署的用户请注意**：把 `ENCRYPTION_KEY` 换成 `openssl rand -base64 32` 的新值后重新部署即可（Cloudflare 在面板改 Secret，Docker 改 `.env` 后重启）。**不会丢任何数据**——坏密钥下加密从未成功执行过，库里不存在用它加密的密文。若你原本用的是**合法**密钥，则完全无需改动；此时**不要**更换密钥，否则已加密的存量字段将无法解密。
